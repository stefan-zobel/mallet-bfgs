...
import mallet.bfgs.MatrixOps;
import mallet.bfgs.Optimizable;

/**
 * Contains static methods for testing subclasses of Maximizable and
 * Maximizable.ByGradient. Especially useful are methods that verify the
 * consistency of the value and gradient functions of an instance of
...
public class TestOptimizable extends TestCase {
    private static final Logger logger = Logger.getLogger(TestOptimizable.class.getName());
    ...
    /**
     * Sets the number of gradient components that will be checked. If negative,
     * all will be checked.
     */
     ...
     * 
     * @param maxable
     *            Instance of a Maximizable that should be tested. Its current
    ...
    public static double testValueAndGradientInDirection(Optimizable.ByGradientValue maxable, double[] direction) {
        ...
        System.out.println("value=" + value + " epsilon=" + epsilon + " epsValue=" + epsValue + " slope = " + slope
                + " gradient=" + directionGradient);
        assert (!Double.isNaN(slope));
        double slopeDifference = Math.abs(slope - directionGradient);
        logger.info("TestMaximizable " + ": slope tolerance = " + tolerance + ": gradient slope = " + directionGradient
                + ", value+epsilon slope = " + slope + ": slope difference = " + slopeDifference);
        maxable.setParameters(oldParameters);
        assert (Math.abs(slopeDifference) < tolerance) : "Slope difference " + slopeDifference
                + " is greater than tolerance " + tolerance;
        return slopeDifference;
    }

    /**
     * Tests that the value and gradient function are consistent at the current
     * parameters. Computes both the analytic gradient (the one given by
     * <tt>maxable.getValueGradient</tt>) and the empirical gradient, which is
     * (if x are the current parameters and f the function computed by maxable)
     * <tt>f(x + epsilon) - f(x)</tt>. Verifies that the angle between the
     * empirical and analytic gradients are close to 0.
     * 
     * @see #testValueAndGradient testValueAndGradient
     * @see #testValueAndGradientRandomParameters
     *      testValueAndGradientRandomParameters
     * @throws IllegalStateException
     *             If the angle is above the tolerance
     */
    public static double testValueAndGradientCurrentParameters(Optimizable.ByGradientValue maxable) {
        ...
        for (int i = 0; i < parameters.length; i++) {
            // { int i = 0; // Uncomment this line to debug one parameter at a
            // time -cas
            if ((parameters.length >= sampleParameterInterval) && (i % sampleParameterInterval != 0))
                continue;
            ...
            double epsValue = maxable.getValue();
            double slope = (epsValue - value) / epsilon;
            System.out.println("value=" + value + " epsValue=" + epsValue + " slope[" + i + "] = " + slope
                    + " gradient[]=" + analyticGradient[i]);
            assert (!Double.isNaN(slope));
            logger.info("TestMaximizable checking singleIndex " + i + ": gradient slope = " + analyticGradient[i]
                    + ", value+epsilon slope = " + slope + ": slope difference = " + (slope - analyticGradient[i]));
    ...
    private static boolean almostEquals(double d1, double d2) {
    ...
    /**
     * Tests that getValue and getValueGradient are consistent. Tests for
     * consistency at <tt>params = 0</tt> and at
     * <tt> params = -0.0001 * grad(f)</tt>
     * 
     * @see #testValueAndGradientCurrentParameters
    ...
    /**
     * Tests that getValue and getValueGradient are consistent at a random
     * parameter setting.
     * 
     * @see #testValueAndGradientCurrentParameters
     *      testValueAndGradientCurrentParameters
