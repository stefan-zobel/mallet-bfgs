 ...
 */
public final class GradientBracketLineOptimizer implements LineOptimizer {
    private static final Logger logger = Logger
            .getLogger(GradientBracketLineOptimizer.class.getName());
    ...
    public double optimize(double[] line, double initialStep) {
        ...
        tx = ax = bx = cx = ox = 0;
        ty = ay = by = cy = optimizable.getValue();
        ...
        do {
            ...
            // logger.info
            // ("Gradient cross-over search, incrementing by "+(tx-ox));
        ...
        while (by <= ay || by <= cy || bx == ax) {
            ...
            if ((Math.abs(bg) < 100 || Math.abs(ay - by) < 10 || Math.abs(by
                    - cy) < 10)
                    && bx != ax)
                // if ((Math.abs(bg) < 10 || Math.abs(ay-by) < 1 ||
                // Math.abs(by-cy) < 1) && bx != ax)
            ...
            // logger.info
            // ("  During min bx search ("+iterations+") ax="+ax+" bx="+bx+" cx="+cx);
            // logger.info
            // ("  During min bx search ("+iterations+") ay="+ay+" by="+by+" cy="+cy);
            // logger.info
            // ("  During min bx search ("+iterations+") ag="+ag+" bg="+bg+" cg="+cg);
        }

        // We now have two points (ax, cx) that straddle the minimum, and a
        // mid-point bx with a value lower than either ay or cy.
        tx = ax
